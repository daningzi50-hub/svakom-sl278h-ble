...
PRODUCT = "84"
PRODUCT_NAME = "SL278H"
MAX_STRENGTH = 8      # 实测上限。超过此值，设备会静默丢弃整条指令
...
def build_control_cmd(func, mode=0, strength=0, heat=False):
    ...
    if func != 3:
        strength = 0
    # 2026-08-01 实测：厂商教程写的 0-20 是整条产品线的通用说法，单机实际区间低得多。
    # 抓包所见 App 发出的振动强度为 6，与实测有效区间 5-8 吻合，上限按 8 收敛。
...
def build_heat_cmd(on, device="main"):
    ...
    主机(振动棒)：开 85-5-1-55-1-0-0，关 85-5-0-0-0-0-0（抄自 App 报文）。
    拍打器：温度挂在自己的功能位 7 上，开 85-7-1-55-1-0-0。
    2026-08-01 实测确认：温度和动作必须分成两条发，同一条里带温度，动作就被丢掉；
    但分开发之后互不干扰——一条开温度，再一条开动作，热着打，两件事同时成立。
    关拍打器用 85-7-0-0-0-0-0，对应 App 拍打区域内温度计图标下方的"关"按钮，
    温度和动作一起断。
    """
    if device == "pat":
        return "85-7-1-55-1-0-0" if on else "85-7-0-0-0-0-0"
    return "85-5-1-55-1-0-0" if on else "85-5-0-0-0-0-0"
