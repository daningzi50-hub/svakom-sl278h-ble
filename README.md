...
> **Brand-agnostic how-to → [ai-toy-bridge](https://github.com/daningzi50-hub/ai-toy-bridge)**
> A separate repo with a generic guide (in Chinese) for wiring *any* remote-capable toy up
> to an AI assistant: which of the three integration routes to pick, how to sniff the protocol
> instead of guessing it, the four silent-failure traps common to this class of device, and how
> to hand over control in a way that's actually safe. Written to be handed to the AI, not the human.
> This repo is the reference implementation it points back to.
> 通用版部署教程已独立成库：把任意一台带远程功能的玩具接给你的 AI，不绑品牌、不绑模型。
